...
; 该文件是Along 32库的一部分。
;
; Along 32库是自由软件：您可以根据《GNU Lesser General Public License》许可证的第3版，
; 或（根据您的选择）任何更高版本的条款重新分发和/或修改它，作为由自由软件基金会发布。
;
; Along32库分发的目的是希望它能对你有用的同时，又不需承担任何保证; 甚至不作适销性或适用于某一特定用途的默示保证。
; 欲了解更多详细信息，查看GNU较宽松通用公共许可证 (General Public License)。
;
; 您应该已经收到了一份NU Lesser General Public许可证的副本及Along32 lib。
...
; 2009/05/25: 修改该文件的主体
; 2009/08/19: 添加注释
; 2010/04/15: 修复ReadInt中的错误和ReadHex中的错误。ReadHex将生成分段错误。
;
; 该库由Curtis Wong创建，用于实验《汇编语言（基于intel x86处理器） - Kip R. Irvine》第4/5版
; 修改自Irvine32.asm
;
; 函数原型
;
...
;
; -------------- 局部函数 -----------
; AsciiDigit : 将数字转换为ascii表示法
; HexByte : 以十六进制显示AL中的字节
...
Clrscr:
;
; 首先，写入控制字符到stdout用于清屏， 然后将光标移至屏幕上的 0，0 处。
;
;-----------------------------------------------------
segment   .data
clrStr db ESC, "[2J", 0
